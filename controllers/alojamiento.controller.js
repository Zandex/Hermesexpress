...
const alojamientotModel = require('../models/alojamiento');
...
const AlojamientoController = {
    ...
    saveProjet: function (req, res) {
        ...
        let project = new alojamientotModel();
    ...
    getProject: (req, res) => {
        ...
        alojamientotModel.findById(projectID, (err, project) => {
            if (err) return res.status(500).send({
    ...
    getProjects: (req, res) => {
        alojamientotModel.find({}).exec((err, projects) => {
            if (err) {
    ...
    updateProject: (req, res) => {
        ...
        alojamientotModel.findByIdAndUpdate(projectID, update, { new: true }, (err, projectUpdate) => {
            if (err) return res.status(500).send({
    ...
    deleteProject: (req, res) => {
        let projectID = req.params.id;
        alojamientotModel.findByIdAndDelete(projectID,(err, projectDeleted)=>{
            if(err) return res.status(500).send({
