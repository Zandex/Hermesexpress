...
const UserController = {
    ...
    getProject: (req, res) => {
        ...
        usertModel.findById(projectID, (err, project) => {
            if (err) return res.status(500).send({
    ...
    getProjects: (req, res) => {
        usertModel.find({}).exec((err, projects) => {
            if (err) {
    ...
    updateProject: (req, res) => {
        ...
        usertModel.findByIdAndUpdate(projectID, update, { new: true }, (err, projectUpdate) => {
            if (err) return res.status(500).send({
    ...
    deleteProject: (req, res) => {
        let projectID = req.params.id;
        usertModel.findByIdAndDelete(projectID,(err, projectDeleted)=>{
            if(err) return res.status(500).send({
